...
export const contractsReducer = (state=initialState,action) => {
	switch (action.type) {
		...
		return {
			...state, dataContracts: action.dataContracts,loading: false, hasErrored: false, message: 'ok',  id:'',	formData: ''
		}
		case types.GET_CONTRACTS_ERROR: 
		return {
			...state,loading: false, hasErrored: true, message: action.error,  id:'',	formData: ''
		}
		case types.GET_CONTRACTS_NOT_FOUND: 
		return {
			...state, dataContracts: '' ,loading: false, hasErrored: false, message: 'notFound',  id:'',	formData: ''
		}
		...
		{
		return {
			...state,dataContracts:state.data.map((item,i)=>{if (JSON.stringify(item)===JSON.stringify(state.id)) {return  action.formData}; return item; }), loading: false, hasErrored: false, id: action.id,	formData: action.formData
		}
		}
